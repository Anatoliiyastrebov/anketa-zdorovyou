...
**ВАЖНО:** Для работы отправки анкет на Netlify необходимо настроить переменные окружения:

1. Перейдите в настройки вашего сайта на Netlify: **Site settings** → **Environment variables**
2. Добавьте следующие переменные:
   - `VITE_TELEGRAM_BOT_TOKEN` = ваш Telegram Bot Token
   - `VITE_TELEGRAM_CHAT_ID` = ваш Telegram Chat ID
3. После добавления переменных **обязательно пересоберите сайт** (Deploys → Trigger deploy → Clear cache and deploy site)

**Примечание:** Если переменные окружения не настроены, отправка анкет не будет работать, и пользователи увидят сообщение об ошибке.
